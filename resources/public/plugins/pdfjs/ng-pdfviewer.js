...
directive('pdfviewer', [ '$parse', function($parse) {
	...
	controller: [ '$scope', function($scope) {
		...
		$scope.pdfDoc.getPage(num).then(function(page) {
		    ...
		    canvas.width = $scope.iElement.parent().width();
                    canvas.height = ($scope.resx/$scope.resy)*canvas.width
			...
			function() { 
			    ...
			    $scope.$apply(function() {
                                $('canvas').parent().find('span.loading').remove()
                                $('canvas').show()
	...
	link: function(scope, iElement, iAttr) {
	    canvas = iElement.find('canvas')[0];
            $('canvas').parent().append('<span class=loading >Loading Document...</span>')
            $('canvas').hide()
